...
{
    public class UserInfo1Controller : MyBaseController//Controller
    {
        ...
        public ActionResult GetPageList(int page,int rows)
        {
            ...
            var result = UserInfoService.GetPageList<int>(where.GetExpression(), u => u.UserId, rows, page, out total).Select(u=>new
            {
                u.UserId,
                u.UserName
            });
        ...
        {
            UserInfo u = UserInfoService.GetById(id1);
        ...
        [HttpPost]
        public ActionResult SetAction(int uid, int aid, int allow)
        {
            string result = "no";
